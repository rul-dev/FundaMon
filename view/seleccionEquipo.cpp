...
void MostrarPokemones()
{
    ...
    for (int i = 0; i < 30; i++)
    {
        //ciclo para recorrer el array y asociarlo al struct de pokemones
        ...
        if ((i + 1) % 3 == 0)
        {
            ...
            // Si no es el último elemento, se agregara un margen a la fila sig
            if (i < 29) 
            {
                cout << string(70, ' ');
            } else if (i==29){
...
void GuardarEquipoPokemon(Equipo &equipoDestino)
{
    ...
    cout << string(70, ' ')<<"Felicidades Tu equipo esta completo\n" << endl;
}